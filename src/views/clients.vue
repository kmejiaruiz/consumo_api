<template>
  <header class="header">
    Buscar pokemon mediante Id o nombre
    <label for="pokemonInput">
      <input id="pokemonInput" type="text" v-model="pokemonID" />
      <button class="searchButton" @click="searchPoke">Buscar Pokemon</button>
    </label>
  </header>
...
export default {
  ...
  methods: {
    async searchPoke() {
      try {
        const pokeFind = await fetch(`${pokeapi}/${this.pokemonID}`);
        const pokemon = await pokeFind.json();
